...
def process_midi(tracks):
    ...
    for x, msg in enumerate(tracks[TRACK]):
        # print(x, ":", msg)
        ...
        if not msg.is_meta:
            if msg.type == 'note_on' and msg.velocity != 0:
                ...
                for i in range(x + 1, len(mid.tracks[TRACK])):
                    next_message = mid.tracks[TRACK][i]
                ...
                new_track.append(mido.Message('note_on', channel = msg.channel, note = msg.note, velocity = msg.velocity, time = 1))
                new_track.append(mido.Message('note_on', channel = msg.channel, note = msg.note, velocity = 0, time = mido.second2tick(0.5, mid.ticks_per_beat, tempo)))
